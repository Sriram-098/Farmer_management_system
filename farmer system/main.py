...
#app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:Sriram@098@localhost/farmers'
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:Sriram@098@localhost/farmers'

app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db = SQLAlchemy(app)

# Set up Flask-Login for user sessions
...
def addagroproduct():
    if request.method == "POST":
        ...
        product = Addagroproducts(username=username, email=email, productname=productname, productdesc=productdesc, price=price)
...
def addfarming():
    if request.method == "POST":
        ...
        existing_farming = Farming.query.filter_by(farmingtype=farmingtype).first()
        if existing_farming:
...
def edit(rid):
    ...
    return render_template('edit.html', post=post, farming=farming)
...
def register():
    ...
    if request.method == "POST":
        ...
        address = request.form.get('address')
        farmingtype = request.form.get('farmingtype')
        new_register = Register(farmername=farmername, adharnumber=adharnumber, age=age, gender=gender, phonenumber=phonenumber, address=address, farming=farmingtype)
